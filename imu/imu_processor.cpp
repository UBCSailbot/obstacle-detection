...
{
	const int timeStep = getTimeStepInMilliseconds(data);
...
void ImuProcessor::MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, int timeStepMillis) {    
    ...
    q0 += qDot1 * (timeStepMillis / 1000.0f);
    q1 += qDot2 * (timeStepMillis / 1000.0f);
    q2 += qDot3 * (timeStepMillis / 1000.0f);
    q3 += qDot4 * (timeStepMillis / 1000.0f);
...
void ImuProcessor::MadgwickAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az, int timeStep) {
    ...
    // Integrate rate of change of quaternion to yield quaternion
    q0 += qDot1 * (timeStep / 1000.0f);
    q1 += qDot2 * (timeStep / 1000.0f);
    q2 += qDot3 * (timeStep / 1000.0f);
    q3 += qDot4 * (timeStep / 1000.0f);
