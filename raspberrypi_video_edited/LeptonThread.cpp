...
void LeptonThread::run()
{
	...
	myMatrix = cv::Mat(80, 60, CV_8UC3); 
	cv::VideoWriter vid = cv::VideoWriter("out.avi", CV_FOURCC('M','J','P','G'),10, cv::Size(80, 60),true);
	...
	while(true) {
		...
		for(int i=0;i<FRAME_SIZE_UINT16;i++) {
			...
			myMatrix.at<cv::Vec3b>(column, row) = cv::Vec3b(colormap[3*value], colormap[3*value+1], colormap[3*value+2]);
		}
